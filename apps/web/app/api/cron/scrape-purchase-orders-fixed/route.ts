...
const SCHEDULE_KEY = 'scrape_purchase_orders';
...
async function handle(req: Request) {
  ...
  // Fallback defaults
  const schedule = scheduleRow ?? { enabled: true, hours: [7, 12, 15], days_of_week: null, config: {} };
  ...
  if (schedule.days_of_week !== null && !schedule.days_of_week.includes(cph.dayOfWeek) && !force) {
  ...
  if ((!isScheduledHour || !isInWindow) && !force) {
