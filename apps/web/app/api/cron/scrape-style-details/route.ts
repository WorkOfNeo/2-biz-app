...
async function handle(req: Request) {
  ...
  // Check if the target season is frozen - skip if so
  if (seasonId) {
    try {
  ...
  // Enqueue scrape_statistics job with style_details enabled
  const insertBody = {
    type: 'scrape_statistics',
    payload: { 
      requestedBy: 'cron-style-details', 
      toggles: { deep: true, style_details: true }, 
      ...(seasonId ? { seasonId } : {}) 
    },
  ...
  await supabase.from('job_logs').insert({ job_id: jobId, level: 'info', msg: 'Enqueued via cron', data: { kind: 'scrape_style_details' } });
  const res = { jobId, note: 'Style details scrape enqueued (only scrapes new customers or those with force_rescrape)' };
  return new Response(JSON.stringify(debug ? { ...res, debug: true } : res), { status: 200, headers: { 'Content-Type': 'application/json' } });
}

export async function POST(req: Request) { try { return await handle(req); } catch (err: any) { return new Response(JSON.stringify({ error: err?.message || 'Cron scrape-style-details error' }), { status: 500 }); } }
export async function GET(req: Request) { try { return await handle(req); } catch (err: any) { return new Response(JSON.stringify({ error: err?.message || 'Cron scrape-style-details error' }), { status: 500 }); } }
