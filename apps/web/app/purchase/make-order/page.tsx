...
function Step3EnterQuantities({
  ...
  // State for which fill option is selected per item
  const [selectedFillOption, setSelectedFillOption] = React.useState<Record<string, 1 | 2 | 3>>({});
        ...
        <CardContent className="space-y-6">
          ...
          <div className="pt-4 border-t-2 border-slate-200">
            <div className="bg-slate-50 rounded-lg border border-slate-200 p-4 space-y-3">
              ...
              {/* Radio buttons for selecting fill option */}
              <div className="flex gap-3">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="radio"
                    name={`fillOption-${key}`}
                    checked={(selectedFillOption[key] || 1) === 1}
                    onChange={() => setSelectedFillOption((prev) => ({ ...prev, [key]: 1 }))}
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="radio"
                    name={`fillOption-${key}`}
                    checked={selectedFillOption[key] === 2}
                    onChange={() => setSelectedFillOption((prev) => ({ ...prev, [key]: 2 }))}
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="radio"
                    name={`fillOption-${key}`}
                    checked={selectedFillOption[key] === 3}
                    onChange={() => setSelectedFillOption((prev) => ({ ...prev, [key]: 3 }))}
